...
**触摸画面用于移动鼠标，点击通过虚拟鼠标按钮完成。** Web 端的手势与原生 iOS 客户端不同。

| 操作 | 使用方式 |
| --- | --- |
| 精准模式 | 在控制栏选择“鼠标模式 → 精准”，触摸位置对应远端鼠标位置 |
| 增量模式 | 选择“增量”，在画面上滑动，以相对位移移动鼠标 |
| 左键 / 右键 | 先定位鼠标，再点虚拟鼠标的“左键”或“右键” |
...
| 手机上点画面没有点击效果 | 画面触摸只移动鼠标，使用虚拟鼠标“左键” / “右键”点击 |
...
这些检查覆盖凭据解析与脚本语法，不能替代真实设备上的连接、音视频和输入测试。修改连接或触控逻辑后，应使用同一服务上的桌面端验证直连 / TURN、画面切换、断开重连和对应输入设备。
